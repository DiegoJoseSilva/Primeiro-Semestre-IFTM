programa {
	...
	funcao kartLocados(cadeia nome [], inteiro status [], inteiro l){
		inteiro i=0
		se (l==0){
			escreva ("Ainda não temos Kards Locados.")
		}senao{
			para (i=0;i<=14; i++){
	...
	funcao menu(inteiro opcao, inteiro &interruptor, inteiro &qtdTotal, cadeia &modelo[], real &valor[], inteiro &qtdLocado[], real &valorAcumulado[], inteiro &disponivel[], inteiro &locados) {
		escolha(opcao) {
			caso 1:
			...
			caso 2:
				...
				pare
			caso 3:
				kartLocados(modelo, disponivel, locados)
				pare
			caso 4:
				alugarKart()
				pare
	...
	funcao inicio(){
		...
		enquanto(interruptor == 1) {
			...
			se(continuar >= 1 e continuar <=10) {
				menu(continuar, interruptor, qtdTotal, modelo, valor, qtdLocado, valorAcumulado, disponivel, locados)
			} senao {
				sair(interruptor)
			}
		}
 ...
 * @POSICAO-CURSOR = 2354; 
 * @PONTOS-DE-PARADA = ;
 * @SIMBOLOS-INSPECIONADOS = {modelo, 127, 9, 6}-{valor, 128, 7, 5}-{qtdLocado, 129, 10, 9}-{disponivel, 130, 10, 10};
